 ...
 */
class utils {
    async addUser(username = "live", password = "evolution", fullName = "", roomNumber = "", workPhone = "", homePhone = "") {
        let result;
        let usrCommand = `sudo adduser --home /home/${username} \
                                  --shell /bin/bash \
                                  --disabled-password \
                                  --gecos "${fullName}","${roomNumber}","${workPhone}","${homePhone}" \
                                  ${username}`;
        result = shelljs_1.default.exec(usrCommand);
         ...
         */
        let pwdCommand = `echo ${username}:${password} | chpasswd `;
        shelljs_1.default.exec(pwdCommand);
        console.log(pwdCommand);
        let sudoCommand = `addgroup ${username} sudo`;
        shelljs_1.default.exec(sudoCommand);
        console.log(sudoCommand);
    }
    async changePassword(username = "live", newPassword = "evolution") {
        let pwdCommand = `echo ${username}:${newPassword} | chpasswd `;
        shelljs_1.default.exec(pwdCommand);
        console.log(pwdCommand);
    }
    async delUser(username = "live") {
        let delCommand = `deluser ${username}`;
        shelljs_1.default.exec(delCommand);
        console.log(delCommand);
    }
    async isLive() {
        let result = false;
        if (fs_1.default.existsSync(`/lib/live/mount`)) {
            result = true;
        }
        return result;
    }
    ...
    bashWrite(file, text) {
        const head = `########################################################START##\n# penguins-eggs: ${path_1.default.basename(file.trim())}\n###############################################################\n`;
        const footer = `######################################################## END ##
    `;
        console.log(`[utils]\n>>> Creazione ${file}`);
        text = head + text.trim() + "\n" + footer;
