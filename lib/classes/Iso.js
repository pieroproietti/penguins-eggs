 ...
 */
class Iso {
    constructor(app, workDir, distro, user, root, net) {
        this.app = {};
        this.distro = {};
        this.user = {};
        this.root = {};
    ...
    async produce(o, c, force = false) {
        if (await utils_1.default.isLive()) {
            console.log(">>> eggs: This is a live system! An egg cannot be produced from an egg!");
        }
        else {
            force = true;
        }
    ...
    async liveDhcp() {
        ...
        utils_1.default.bashWrite(`${this.distro.pathFs}/etc/network/interfaces`, text);
    }
    /**
     * eggCreateStructue
     */
    async eggCreateStructure() {
        console.log("==========================================");
    ...
    async eggSystemCopy() {
        let specificFilters;
        let cmd = "";
        specificFilters = filters_1.default + ` --filter="+ /home/${this.user.name}" --filter="- /home/*"`;
        cmd = `
      rsync -aq  \
    ...
    async kill() {
        ...
        console.log("==========================================");
        utils_1.default.exec(`rm -rf ${this.workDir}`);
    ...
    async isolinuxPrepare(o) {
        console.log("==========================================");
        console.log("iso: isolinuxPrepare");
        console.log("==========================================");
        let isolinuxbin = `${o.isolinuxPath}isolinux.bin`;
        let vesamenu = `${o.syslinuxPath}vesamenu.c32`;
        utils_1.default.exec(`rsync -a ${o.syslinuxPath}chain.c32 ${this.distro.pathIso}/isolinux/`);
        utils_1.default.exec(`rsync -a ${o.syslinuxPath}ldlinux.c32 ${this.distro.pathIso}/isolinux/`);
        utils_1.default.exec(`rsync -a ${o.syslinuxPath}libcom32.c32 ${this.distro.pathIso}/isolinux/`);
        utils_1.default.exec(`rsync -a ${o.syslinuxPath}libutil.c32 ${this.distro.pathIso}/isolinux/`);
...
`;
    ...
    async menuCfg(o) {
        let kernel = utils_1.default.kernerlVersion();
    ...
    LABEL ${this.distro.name} safe
      ...
      ${o.appendSafe}`;
        utils_1.default.bashWrite(file, text);
        utils_1.default.exec(`cp ${__dirname}/../../assets/penguins-eggs-3-syslinux.png ${this.distro.pathIso}/isolinux`);
    }
    /**
     * alive: rende live
     */
    ...
    async makeIsoFs(o) {
        ...
        let volid = o.distroName + utils_1.default.date4file();
