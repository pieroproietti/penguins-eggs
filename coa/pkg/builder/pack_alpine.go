...
`, pkgVersion, timestamp)
	...
	// 8. Impacchettamento nativo: un .apk non firmato è un tar.gz con .PKGINFO in root.
	tmpApkLocation := filepath.Join(ctx.BaseBuildDir, apkFile)

	tarCmd := exec.Command("tar", "-czf", tmpApkLocation, "-C", buildDir, ".")
	tarCmd.Stdout, tarCmd.Stderr = os.Stdout, os.Stderr
	if err := tarCmd.Run(); err != nil {
		LogError("Failed to build Alpine package (tar): %v", err)
		return
	}
