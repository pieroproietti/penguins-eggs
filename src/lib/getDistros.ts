...
read('/etc/os-release', function (data) {
  for (var temp in data) {
    if (!data[temp].search(os['PRETTY_NAME'])) {
      os['PRETTY_NAME'] = data[temp].substring(os['PRETTY_NAME'].length);
    };

    if (!data[temp].search(os['NAME'])) {
      os['NAME'] = data[temp].substring(os['NAME'].length, 100);
    };

    if (!data[temp].search(os['ID'])) {
      os['ID'] = data[temp].substring(os['ID'].length);
    };

    if (!data[temp].search(os['VERSION_ID'])) {
      os['VERSION_ID'] = data[temp].substring(os['VERSION_ID'].length);
    };

    if (!data[temp].search(os[`HOME_URL`])) {
      os[`HOME_URL`] = data[temp].substring(os[`HOME_URL`].length);
    };

    if (!data[temp].search(os[`SUPPORT_URL`])) {
      os[`SUPPORT_URL`] = data[temp].substring(os[`SUPPORT_URL`].length);
    };

    if (!data[temp].search(os[`BUG_REPORT_URL`])) {
      os[`BUG_REPORT_URL`] = data[temp].substring(os[`BUG_REPORT_URL`].length);
    };
  }
  var oJson = "{ ";
