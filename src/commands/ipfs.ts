 ...
 * license: MIT
 *
 * Integration: manage IPFS distribution of eggs ISOs.
 */
...
export default class Ipfs extends Command {
  static description = 'manage IPFS distribution of eggs ISOs via brig'

  static examples = [
    'sudo eggs ipfs publish /home/eggs/egg-debian.iso',
    'eggs ipfs list',
    'eggs ipfs get /isos/egg-debian.iso --dest /tmp/',
    'eggs ipfs gateway',
  ...
  static args = {
    action: Args.string({
      description: 'action: publish, list, get, gateway, history, enable, disable, setup',
      options: ['publish', 'list', 'get', 'gateway', 'history', 'enable', 'disable', 'setup'],
      required: true,
    }),
    path: Args.string({
      description: 'ISO path (publish), brig path (get/history)',
      required: false,
    }),
  }
  ...
  async run(): Promise<void> {
    ...
    const { BrigPublisher, loadIpfsConfig, saveIpfsConfig } = await import('penguins-eggs-integrations/decentralized')
    ...
    switch (args.action) {
      case 'publish': {
        ...
        this.log(`Publishing ${args.path} to IPFS...`)
        ...
        if (result.gatewayUrl) this.log(`Gateway: ${result.gatewayUrl}`)
        break
      }
      ...
      case 'history': {
        ...
        for (const entry of history) this.log(entry)
        break
      }
