...
export default class SysUsers {
  ...
  public async save() {
    ...
    // File raw
    if (this.gshadowLines.length > 0) await this.writeFile('etc/gshadow', this.gshadowLines.join('\n'), 'shadow_t')

    if (this.subuidLines.length > 0) await this.writeFile('etc/subuid', this.subuidLines.join('\n'), 'passwd_file_t')

    if (this.subgidLines.length > 0) await this.writeFile('etc/subgid', this.subgidLines.join('\n'), 'passwd_file_t')
  }
