...
export default class Xdg {
  ...
  static async autologin(olduser: string, newuser: string, chroot = '/') {
    if (await Pacman.isInstalledGui()) {
      ...
      // gdm if Archlinux family 
      if (Pacman.packageIsInstalled('gdm')) {
      	const dirConf = `${chroot}/etc/gdm`
        const autologin = `${dirConf}/custom.conf`
        if (fs.existsSync(autologin)) {
          const content = fs.readFileSync(autologin)
          if (content.includes('AutomaticLoginEnable')) {
		  shx.sed('-i', 'AutomaticLoginEnable=False', 'AutomaticLoginEnable=True', `${chroot}/etc/gdm/custom.conf`)
		  shx.sed('-i', `AutomaticLogin=${olduser}`, `AutomaticLogin=${newuser}`, `${chroot}/etc/gdm/custom.conf`)
      } else {
      	const content = `AutomaticLoginEnable=True\nAutomaticLogin=${newuser}\n`
      	fs.writeFileSync(autologin, content, 'utf-8')
      }
    }
  }
  ...
  static async skel(user: string, verbose = false) {
    ...
    if (Pacman.packageIsInstalled('cinnamon-core')) {
      ...
      desktop = 'lxqt'
    } else if (Pacman.packageIsInstalled('gnome-shell')) {
      desktop = 'gnome'
    }
    ...
    // await execIfExist(`rm -rf`, `/etc/skel/.config/gtk-4.0`, verbose)
