...
export async function makeSquashfs(this: Ovary, scriptOnly = false, unsecure = false): Promise<string> {
     ...
     */
    let cmd = `strace -e trace=open,read,write,close mksquashfs -xattrs -info  ${this.settings.work_dir.merged} ${this.settings.iso_work}live/filesystem.squashfs ${compression} ${limit} -wildcards -ef ${this.settings.config.snapshot_excludes} ${this.settings.session_excludes}`
