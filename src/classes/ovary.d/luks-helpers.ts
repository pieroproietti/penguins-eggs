...
import { type ArgonCryptoConfig, type CryptoConfig, interactiveCryptoConfig, type Pbkdf2CryptoConfig } from './luks-interactive-crypto-config.js' // Assicurati che il percorso sia corretto
 ...
 */
export function luksExecuteCommand(this: Ovary, command: string, args: string[], stdinData?: string): Promise<void> {
  if (!this.hidden) {
 ...
 * buildLuksFormatArgs
 */
export function buildLuksFormatArgs(this: Ovary, config: CryptoConfig, luksFile: string): string[] {
  const args: string[] = [
    ...
    config.cipher,
    '--key-size',
    config['key-size'].toString(),
    '--hash',
    config.hash,
    '--sector-size',
    config['sector-size'].toString(),
    '--pbkdf',
    config.pbkdf
  ]
