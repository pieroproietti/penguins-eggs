...
export async function initrdDracut(this: Ovary) {
    ...
    await exec(`dracut --force --debug --no-hostonly --confdir ${confdir} ${this.settings.iso_work}live/${path.basename(this.initrd)}  ${this.kernel} > /var/log/penguins-eggs-dracut-output.log 2>&1`, this.echo)
}
