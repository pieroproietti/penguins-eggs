...
export default async function biosLuks(this: Sequence, installDevice = "", p = ""): Promise<boolean> {
    ...
    // ROOT
    try {
        await exec(`echo -n "${passphrase}" | cryptsetup --batch-mode --cipher ${cipher} --key-size ${keySize} --hash ${hash} --type luks2 --key-file=- -v luksFormat ${installDevice}${p}2`, this.echo)
    } catch (error: any) {
    ...
    try {
        await exec(`echo -n "${passphrase}" | cryptsetup --key-file=- --type luks2 luksOpen ${installDevice}${p}2 root_crypted`, this.echo)
    } catch (error: any) {
        Utils.warning(`Error: ${error.message}`)
        process.exit(1)
    }
