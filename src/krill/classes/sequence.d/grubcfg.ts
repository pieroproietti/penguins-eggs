...
export default async function grubcfg(this: Sequence) {
  ...
  for (let line of grubs) {
    ...
    if (
      isRhelFamily &&
      (line.trim().startsWith('GRUB_CMDLINE_LINUX_DEFAULT=') || line.trim().startsWith('GRUB_CMDLINE_LINUX=')) && // Se non c'è già il parametro, lo iniettiamo subito dopo la prima virgoletta
