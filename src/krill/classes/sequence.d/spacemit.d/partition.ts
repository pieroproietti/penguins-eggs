...
export default async function partition(this: Sequence): Promise<boolean> {
  ...
  try {
    ...
    // 5. Scrittura binari Bootloader nelle partizioni RAW
    // Definiamo il percorso dove eggs conserva i binari firmware

    const firmwarePath = path.resolve(__dirname, '../../../../../spacemit/')

    console.log("Staging bootloader binaries to RAW partitions...");

    await exec(`dd if=${firmwarePath}/factory/FSBL.bin of=${device}${p}1 status=none`);
    await exec(`dd if=${firmwarePath}/env.bin of=${device}${p}2 status=none`);
    await exec(`dd if=${firmwarePath}/fw_dynamic.itb of=${device}${p}3 status=none`);
    await exec(`dd if=${firmwarePath}/u-boot.itb of=${device}${p}4 status=none`);
