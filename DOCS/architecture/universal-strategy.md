...
The **Universal Strategy** is the philosophical and technical framework guiding the development of **penguins-eggs**.
...
### B. Distro modules (`modules/<distro>/`)
These are the "impedance adapters" — currently `alpine`, `arch-family`, `debian`, `fedora`, `opensuse`, divided into `remaster.bash.tmpl` and `install.bash.tmpl`. They translate the universal hooks into the local dialects:
...
Thanks to this architecture, **penguins-eggs** is not tied to the present. The Universal Strategy makes it possible to:
* **Support new distros:** adding a distribution only requires creating a new directory in `modules/<distro>/` with `remaster.bash.tmpl` and `install.bash.tmpl`, without touching the Go core.
