...
The philosophy is the one described in [philosophy.md](../../1-philosophy/2-philosophy.md): `oa` is the site foreman — blind to the overall plan, infallible at its specific task. Everything high-level (SquashFS, ISO generation, bootloaders) belongs to the Go modules executed through `coa ell` — see [ell.md](./4-ell.md).
