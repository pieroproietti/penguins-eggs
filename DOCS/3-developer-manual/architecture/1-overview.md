...
The `cmd` package contains the user interface. Each file maps a command the user can invoke. See the [Command Reference](../../2-user-manual/2-commands.md) for the complete, user-facing documentation; the highlights:

| Command | Sudo | Role |
| :--- | :--- | :--- |
| `remaster` | Yes | The heart of the system. Starts the "flight" that produces the ISO: the `parser` reads the YAML rules, the `planner` writes the JSON plan, then the C binary is launched. Supports `--clone`, `--crypted`, `--wizard` and `--stop-after` for surgical debugging. |
...
This two-phase design (logic and abstraction in **Go** ➔ raw syscall execution in **C**) is what makes **penguins-eggs** a perfect hybrid: the maintainability and flexibility of modern Go, fused with the surgical isolation, speed and low-level safety of C.
