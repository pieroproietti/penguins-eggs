...
Triggered manually (`workflow_dispatch` via `incubator-go.yml`), Incubator leverages **[incubator-go](https://github.com/pieroproietti/incubator)** — a high-performance, zero-dependency Go orchestration tool designed to automatically deploy, install, and rigorously test Linux ISOs generated by `penguins-eggs` on Proxmox VE.
