"""
penguins-kernel-manager CLI entry point.

Combines ukm's runtime management commands with lkf's build pipeline,
all accessible from a single `penguins-kernel-manager` command.

Usage:
  penguins-kernel-manager list [--family=<f>] [--installed] [--json] [--refresh]
  penguins-kernel-manager install [<version>] [--flavor=<f>] [--provider=<p>] [--local=<path>] [--last-build]
  penguins-kernel-manager remove <version> [--purge]
  penguins-kernel-manager hold <version>
  penguins-kernel-manager unhold <version>
  penguins-kernel-manager note <version> <text>
  penguins-kernel-manager remove-old [--keep=<n>] [--purge]
  penguins-kernel-manager providers
  penguins-kernel-manager info
  penguins-kernel-manager build [--version=<v>] [--flavor=<f>] [--arch=<a>] [--llvm] [--lto=<t>] [--output=<fmt>] [--install]
  penguins-kernel-manager remix [--file=<path>] [--install]
  penguins-kernel-manager -h
"""
from __future__ import annotations

import sys

from penguins_kernel_manager.cli.output import die, header, ok, print_json, print_table, warn
from penguins_kernel_manager.core.manager import KernelManager
from penguins_kernel_manager.core.system import system_info
...
def cmd_build(args: dict) -> None:
    ...
    version = args.get("--version") or die("--version is required for penguins-kernel-manager build")
    ...
    if install:
        from penguins_kernel_manager.core.providers.lkf_build import _find_output_package
...
def cmd_remix(args: dict) -> None:
    ...
    profile = args.get("--file") or die("--file is required for penguins-kernel-manager remix")
    install = args.get("--install", False)
...
def main() -> None:
    from penguins_kernel_manager import __version__
    if "--version" in sys.argv:
        print(f"penguins-kernel-manager {__version__}")
        return
