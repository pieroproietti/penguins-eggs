...
~/.config/penguins-kernel-manager/state.json), and exposes a unified API used by both the CLI
and the GUI.
"""
...
from penguins_kernel_manager.core.kernel import KernelEntry, KernelFamily, KernelStatus
from penguins_kernel_manager.core.providers import get_providers
...
from penguins_kernel_manager.hooks import post_install, post_remove_old, pre_install, pre_remove

_STATE_FILE = Path.home() / ".config" / "penguins-kernel-manager" / "state.json"
...
class KernelManager:
    ...
    def install_last_build(self) -> Iterator[str]:
        ...
        if pkg is None:
            raise RuntimeError(
                ...
                "Run 'penguins-kernel-manager build' or 'penguins-kernel-manager remix' first."
            )
    ...
    def nixos_build_warning(self) -> str | None:
        info = system_info()
        from penguins_kernel_manager.core.system import DistroFamily
