...
* **Rigorous Preflight Validations**: Performs extensive preflight checks for UEFI firmware, supported distribution families (Debian, Arch and Manjaro families), storage layout geometry, block device collision detection across all attached disks, and mount safety before any writes occur.
* **Manjaro Coexist Support**: Enabled Manjaro and recognized derivatives, including explicit BigLinux and BigCommunity detection. The shared Arch/Manjaro bootloader dispatcher selects GRUB for Coexist with the chosen EFI identity, preserving sibling EFI directories and `EFI/BOOT`; missing UEFI or a mounted ESP stops bootloader installation. Added simulated template and family-detection coverage; VM boot validation remains pending.
